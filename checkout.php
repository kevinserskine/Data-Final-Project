...
foreach ($_SESSION['cart']['buy'] as $key => $value) {
    ...
    $book_id = $row['book_id'];
    $price = $value['price'];
    $result = mysqli_query($conn, "INSERT INTO user_order (order_date, user_id, dest_address_id) VALUES (NOW(), $userID, 1)");
    echo print_r($result);
...
foreach ($_SESSION['cart']['borrow'] as $key => $value) {
    ...
    $row = mysqli_fetch_assoc($result);
    $book_id = $row['book_id'];
    mysqli_query($conn, "INSERT INTO user_order (order_date, user_id, dest_address_id) VALUES (NOW(), $userID, 1)");
    $order_id = mysqli_insert_id($conn);
